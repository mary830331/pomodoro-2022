...
#list
  b-container
    b-row
      b-col(cols="12")
        b-form-group(label="新增事項" label-for="newinput" invalid-feedback="多打幾個字會死嗎?")
        ...
        b-table(:items="items" :fields="fields" small show-empty hover borderless)
          ...
          template(#cell(action)="data")
            span(v-if="data.item.edit")
              ...
              b-btn(pill size="sm" variant="outline-secondary")
        ...
        b-table-simple(small hover borderless)
          tbody
            ...
            tr(v-if="finished.length === 0")
              td.text-center(colspan="2") 人生黑白了嗎? 不會找點事做嗎?
</template>
<script>
export default {
  data () {
